///////////////////////////////////////////////////////////////////////
////  Copyright 2020 by mars.     实现局部+全局历史信息                 //
///////////////////////////////////////////////////////////////////////
...
void PREDICTOR_init(void)
{
	...
	ghr = 0;
	numPhtEntries = (1 << HIST_LEN);    		// 模式历史表，有2^17项
	numLhtEntries = (1 << (HIST_LEN-14));    	// 局部历史表，有2^14项
...
{
	UINT32 lhtIndex = (PC & 0x3FFF) % (numPhtEntries);			
	UINT32 phtIndex = (((PC & 0x1C000) | lht[lhtIndex]) ^ ghr) % (numPhtEntries);	
...
void  UpdatePredictor(UINT64 PC, OpType opType, char resolveDir, char predDir, UINT64 branchTarget)
{
	...
	UINT32 lhtIndex = (PC & 0x3FFF) % (numPhtEntries);			// 提取出后14位作为lht的索引
	UINT32 phtIndex = (((PC & 0x1C000) | lht[lhtIndex]) ^ ghr) % (numPhtEntries);	// 提取出末尾14位左侧的3位，与lht值或运算
	UINT32 phtCounter = pht[phtIndex];
